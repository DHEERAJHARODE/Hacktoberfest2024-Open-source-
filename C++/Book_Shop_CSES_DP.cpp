...
int rec(int j,int sum,vector<int>& vec,vector<int>& pages, vector<vector<int>>& dp,int budget)
{
   if(j >= vec.size())
      return 0;
   // for(int i = j; j < vec.size(); j++)
   // {
   if(dp[j][sum] != -1)
      return dp[j][sum];
   int &ans = dp[j][sum];
   if(sum + vec[j] <= budget)
      ans = rec(j+1,sum+vec[j],vec,pages,dp,budget) + pages[j];
   // }
   ans = max(rec(j+1,sum,vec,pages,dp,budget),ans);
   return ans;
}
void solve()
{  
    //dp[x][i] represents the max no. of pages for price at most x which can be bought for the ith book'
    int n, k; cin >> n >> k;
     vector<int> vec(n), pages(n);
     for (auto &v : vec) 
      cin >> v;
     for (auto &v : pages) 
      cin >> v;
     vector<vector<int>> dp(n+1,vector<int>(k+1,-1));
     int ans = rec(0,0,vec,pages,dp,k);
     cout << dp[0][0] << endl;
}
int32_t main() 
{
